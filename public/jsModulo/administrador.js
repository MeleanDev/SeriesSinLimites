        ...
        success: function (data) {
            if (data.success) {
                ...
                notificacion.fire({
                    ...
                    text: "Recuerda que no pueden haber 2 Admin con el misma Email."
                });
            }
        },
...
crear = function () {
    ...
    $("#password").attr("readonly", false).show();
    $("#rol").attr("readonly", false).show();
    $("#titlepassword").show();
...
ver = async function (id) {
    try {
        ...
        $("#titulo").html("Ver Alojamiento -> " + datos.name);
        $("#titulo").attr("class", "modal-title text-white");
        ...
        $("#rolinput").val((datos.rol == 1) ? "SuperAdmin" : "Admin");
        $("#rolinput").show();

        $("#titlepassword").hide();
        $("#password").attr("readonly", true).hide();
...
editar = async function (id) {
    ...
    try {
        $("#formulario").trigger("reset");
        datos = await consulta(id);
        $("#titulo").html("Editar Alojamiento -> " + datos.email);
        $("#titulo").attr("class", "modal-title text-white");
...
eliminar = async function (id) {
    datos = await consulta(id);
    Swal.fire({
        title: '¿ Estas seguro que desea eliminar el registro #' + datos.email + ' ?',
